...
# -*- coding: utf-8 -*-
"""
三子棋AI系统 - 第一玩家计算机视觉检测模块

本模块专门用于检测白棋(第一玩家)的棋子放置。
主要功能：
- 通过摄像头捕获棋盘图像
- 使用图像处理技术检测圆形棋子
- 识别白色棋子并确定其在3x3网格中的位置
- 防止重复检测同一位置的棋子
...
def capture_and_process_image(detected_positions):
    ...
    本函数是第一玩家(人类)的棋子检测核心函数，专门检测白色棋子。
    
    参数:
        detected_positions (set): 已检测到的棋子位置集合，用于避免重复检测
    
    返回:
        tuple: (位置, 颜色) 如果检测到新的白棋
        None: 如果未检测到新的白棋或检测失败
    
    检测原理:
    1. 图像预处理：灰度化 -> 二值化 -> 自适应阈值 -> 形态学腐蚀
    2. 霍夫圆检测：检测圆形物体(棋子)
    3. 颜色判断：通过随机采样判断是否为白色棋子
        ...
        for (x, y, r) in circles:
            ...
            if x - r >= 0 and y - r >= 0 and x + r < roi_w and y + r < roi_h:
                ...
                if sample_points:
                    ...
                    # 检查条件：位置未被检测过 且 是白色棋子(第一玩家)
                    if position not in detected_positions and color == "white":
                        cap.release()  # 释放摄像头资源
